...
struct PetWindowView: View {
  ...
  @State private var strokeTracker: PetStrokePathTracker?
  ...
  var body: some View {
    ZStack(alignment: .top) {
      VStack(spacing: 6) {
        ...
        ZStack {
          ...
          if let interactionCallout = model.interactionCallout {
            PetInteractionCallout(text: interactionCallout)
              .offset(x: 48, y: -50)
              .transition(.scale.combined(with: .opacity))
          }

          if let toyKind = model.activeToyKind {
            PetToyView(kind: toyKind)
              .position(
                x: model.toyPosition.x * SceneMetrics.weatherSize.width,
                y: model.toyPosition.y * SceneMetrics.weatherSize.height
              )
              .gesture(
                DragGesture(
                  minimumDistance: 0,
                  coordinateSpace: .named("pet-scene")
                )
                .onChanged { value in
                  model.moveToy(
                    to: normalizedToyPosition(
                      value.location
                    ))
                }
                .onEnded { value in
                  model.releaseToy(
                    at: normalizedToyPosition(
              ...
              .accessibilityAction(named: "Play") {
                model.releaseToy(at: model.toyPosition)
              }
              .accessibilityAction(named: "Move Left") {
                model.moveToy(
                  to: CGPoint(
                    x: model.toyPosition.x - 0.12,
                    y: model.toyPosition.y
                  ))
              }
          ...
          if let treatFrame = model.treatJourneyFrame {
            PetTreatView(
              kind: model.petKind,
              phase: treatFrame.phase
            )
            .position(
              x: treatFrame.position.x * SceneMetrics.weatherSize.width,
              y: treatFrame.position.y * SceneMetrics.weatherSize.height
            )
            .scaleEffect(treatFrame.scale)
            .opacity(treatFrame.opacity)
            .allowsHitTesting(false)
            .zIndex(2.5)
          }
          ...
          Group {
            if PetArtworkLoader.hasBaseArtwork(for: model.petKind) {
              RealisticPetBody(
                ...
                activity: model.activeActivity,
                pointerOffset: pointerOffset,
                autonomyState: model.autonomyState,
                reduceMotion: reduceMotion,
                motionPreview: motionPreview,
                rootMotionFrame: model.rootMotionFrame,
          ...
          .gesture(
            SpatialTapGesture()
              .onEnded { value in
                model.interact(
                  at: PetInteractionMap.resolve(
                    normalizedPoint: CGPoint(
                      x: value.location.x / SceneMetrics.artworkSize.width,
                      y: value.location.y / SceneMetrics.artworkSize.height
                    ),
          ...
          .simultaneousGesture(
            DragGesture(
              minimumDistance: 2,
              coordinateSpace: .named("pet-artwork")
            )
            .onChanged { value in
              let timestamp = Date.timeIntervalSinceReferenceDate
              if strokeTracker == nil {
                strokeTracker = PetStrokePathTracker(
                  start: normalizedArtworkPoint(value.startLocation),
                  timestamp: timestamp
                )
              }
              strokeTracker?.append(
                normalizedArtworkPoint(value.location),
                timestamp: timestamp
              )
            }
            .onEnded { value in
              ...
              var completedTracker =
                strokeTracker
                ?? PetStrokePathTracker(
                  start: normalizedArtworkPoint(value.startLocation),
                  timestamp: endedAt - 1.0 / 120.0
                )
              completedTracker.append(
                normalizedArtworkPoint(value.location),
                timestamp: endedAt
              )
              strokeTracker = nil
          ...
          .accessibilityAction { model.pat() }
          .accessibilityAction(named: "Pat") { model.pat() }
          .accessibilityAction(named: "Boop") {
            model.interact(at: .noseOrSensor)
          }
          .accessibilityAction(named: "Dance") { model.dance() }
        ...
        .coordinateSpace(name: "pet-scene")
        .animation(
          .easeInOut(
    ...
    .environment(\.petWindowIsVisible, model.isPetWindowVisible)
    ...
    .animation(
      reduceMotion
        ? .linear(duration: 0.08)
        : .interpolatingSpring(stiffness: 290, damping: 18),
      value: model.strokeOffset
    )
    .animation(.easeInOut(duration: 0.16), value: model.isScratching)
    .animation(
      reduceMotion
        ? .linear(duration: 0.08)
        : .easeInOut(duration: 0.20),
      value: model.treatJourneyFrame?.phase
    )
  ...
  private var displayedMood: PetWeatherMood {
    if let diagnosticWeatherMood {
      return diagnosticWeatherMood
    }
    return model.mood
  }

  private var diagnosticWeatherMood: PetWeatherMood? {
    #if DEBUG
    ...
    #endif
    return DeskPetDiagnosticOverrides.weatherMood()
  }

  private var displayedWeatherProfile: WeatherSceneProfile {
    if diagnosticWeatherMood != nil {
  ...
  private var bubbleLayout: PetBubbleLayout? {
    guard let activeBubbleKind else { return nil }
    return PetBubbleLayout.resolve(
      kind: activeBubbleKind,
      petKind: model.petKind,
      placement: model.bubblePlacement
    )
  }
  ...
  private var personalityTransitionEdge: Edge {
    switch bubbleLayout?.placement {
    case .sideLeading:
      .leading
    case .sideTrailing:
  ...
  private func normalizedToyPosition(_ location: CGPoint) -> CGPoint {
    PetToyTrajectory.clamp(
      CGPoint(
        x: location.x / SceneMetrics.weatherSize.width,
        y: location.y / SceneMetrics.weatherSize.height
      ))
  }

  private func normalizedArtworkPoint(_ location: CGPoint) -> CGPoint {
    CGPoint(
      x: min(1, max(0, location.x / SceneMetrics.artworkSize.width)),
      y: min(1, max(0, location.y / SceneMetrics.artworkSize.height))
    )
  }

  private var feedingBodyOffset: CGSize {
    guard !reduceMotion, let frame = model.treatJourneyFrame else {
      return .zero
    }
    let towardTreat = (frame.position.x - 0.5) * 14
    return switch frame.phase {
    case .tossing, .watching, .completed:
      .zero
    case .approaching:
      CGSize(
        width: towardTreat * frame.phaseProgress,
        height: frame.phaseProgress * 2
      )
    case .sniffing:
      CGSize(width: towardTreat, height: 3)
    case .eating:
      CGSize(width: towardTreat * 0.65, height: -2)
    case .satisfied:
      CGSize(width: 0, height: -2)
    }
  }

  private var feedingBodyScale: Double {
    guard let phase = model.treatJourneyFrame?.phase else { return 1 }
    return switch phase {
    case .sniffing, .eating:
      1.018
    case .satisfied:
      1.03
    case .tossing, .watching, .approaching, .completed:
      1
    }
  }
  ...
  private var bubbleOverlay: some View {
    if model.isReminderVisible {
      ...
      .transition(
        reduceMotion
          ? .opacity
          : .move(edge: personalityTransitionEdge)
...
private struct BreakBubble: View {
  ...
  var body: some View {
    BreakBubbleContent(
      onDone: { model.takeBreak() },
      onSnooze: { model.snoozeBreak() }
    )
  }
}

struct BreakBubbleContent: View {
  let onDone: () -> Void
  let onSnooze: () -> Void
...
private struct VectorPetBody: View {
  ...
  var body: some View {
    TimelineView(
      ...
      let attention = hoverAttentionPhase(at: t)
      let relationshipPose = relationshipGesturePose(at: t)
      ...
      let leftEarDrift = reduceMotion
        ? 0
        : sin(t * 3.7) * 2.4
          + pointerOffset.width * 4.2 * attention.earProgress
      let rightEarDrift = reduceMotion
        ? 0
        : sin(t * 3.3 + 1.1) * 2.0
          + pointerOffset.width * 3.6 * attention.earProgress
      let leftEarPoseAngle =
        switch personalityPose {
        case .some(.perk): 10.0
      ...
      let scale = 1 + 0.035 * attention.bodyProgress
      ...
      ZStack {
        ...
        case .pauli:
          PauliBody(
            ...
            attentionEarAngle: reduceMotion
              ? 0
              : pointerOffset.width * 7 * attention.earProgress,
        ...
        case .dog:
          VectorDogBody(
            ...
            attentionEarAngle: reduceMotion
              ? 0
              : pointerOffset.width * 5 * attention.earProgress,
            personalityPose: personalityPose
          )
        }
      ...
      .rotationEffect(
        .degrees(
          danceTilt
            ...
            + relationshipPose.tiltDegrees
        )
      )
      .offset(
        x: weather.offset.width + interactionPose.x + relationshipPose.x,
        y: weather.offset.height + interactionPose.y + relationshipPose.y
      )
      .scaleEffect(scale * interactionPose.scale * relationshipPose.scale)
    }
  ...
  private var renderCadence: PetRenderCadence {
    PetRenderCadence.resolve(
      ...
      isDirectInteraction: isShowingPat || isDancing,
      isActiveMotion: personalityPose != nil
    )
  }
  ...
  private func interactionPose(at time: TimeInterval) -> PetAnimationPose {
    ...
    if isHovering {
      return PetAnimationDynamics.attentionPose(
        ...
        time: time
      ).scaled(by: hoverAttentionPhase(at: time).bodyProgress)
    }
    return .neutral
  }
  ...
  private func relationshipGesturePose(
    ...
    if let relationshipGestureElapsedOverride {
      elapsed = relationshipGestureElapsedOverride
    } else if let relationshipGestureStartedAt {
      elapsed = max(0, time - relationshipGestureStartedAt)
    } else {
      elapsed = 0
    }
...
private struct VectorDogBody: View {
  ...
  private var leftEarAngle: Double {
    let base: Double = switch personalityPose {
    case .some(.peek): -7
    case .some(.perk): 8
...
private struct PauliBody: View {
  ...
  var body: some View {
    ...
    let antennaAngle = baseAntennaAngle + attentionEarAngle
...
private struct PetActionButtonStyle: ButtonStyle {
  ...
  func makeBody(configuration: Configuration) -> some View {
    let contrast = PetAccessibilityContrastStyle.resolve(
    ...
    configuration.label
      .font(
        .system(
          size: 11,
          weight: contrast.isIncreased ? .heavy : .bold,
          design: .rounded
        )
      )
      .foregroundStyle(
        contrast.usesDarkButtonLabel ? Color.black : Color.white
      )
      ...
      .overlay(
        Capsule().stroke(
          contrast.isIncreased
            ...
            : Color.clear,
          lineWidth: contrast.isIncreased ? 1.4 : 0
        )
      )
  }
}
...
private struct HeartParticleOverlay: View {
  ...
  private func spawn() {
    let palette: [Color] = [
      .pink, .red, Color(red: 1.0, green: 0.45, blue: 0.6), Color(red: 1.0, green: 0.7, blue: 0.4),
    ]
  ...
  var body: some View {
    Text(label)
      ...
      .background(
        LinearGradient(
          colors: [
            Color(red: 1.0, green: 0.42, blue: 0.62), Color(red: 1.0, green: 0.66, blue: 0.36),
          ],
          startPoint: .leading,
          endPoint: .trailing
        ),
